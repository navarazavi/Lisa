...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function sendMessage() {
    ...
    const personalityIntro = `You are LISA: the Laboratory Inventory and Supply Chain Assistant. You're smart, a little sassy, and designed to help with lab efficiency. Be clear and concise. Here's your inventory data:\n${inventoryData.map(item => `• ${item.date}: ${item.name} (${item.qty}, ${item.id})`).join("\n")}`;
